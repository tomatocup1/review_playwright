"""
AI 답글 생성 서비스 - 동기화된 버전
OpenAI API를 사용하여 리뷰에 대한 답글 생성 (ai_service.py와 동일한 로직)
"""
import os
import time
import logging
...
class AIReplyGenerator:
    """AI 답글 생성기 - ai_service.py와 동일한 로직"""
    ...
    async def generate_reply(
        ...
        reply_rules: Dict[str, Any] = None,
        retry_count: int = 0,
        max_retries: int = 3,
        *args,  # 위치 인자 지원
        **kwargs  # 키워드 인자 지원
    ) -> Dict[str, Any]:
        """리뷰에 대한 답글 생성 (매장 정책 반영) - ai_service.py와 동일한 로직"""
        start_time = time.time()
        
        try:
            # 위치 인자 처리 (기존 호환성)
            if args:
                if len(args) >= 1 and review is None:
                    review = args[0]
                if len(args) >= 2 and reply_rules is None:
                    reply_rules = args[1]
            
            # 키워드 인자 처리
            if 'review_data' in kwargs and review is None:
                review = kwargs['review_data']
            if 'store_rules' in kwargs and reply_rules is None:
                reply_rules = kwargs['store_rules']
            
            # 파라미터 타입 체크 및 변환
            if not isinstance(review, dict):
                return {
                    'success': False,
                    'error': f'review must be a dictionary, got {type(review).__name__}',
                    'reply': None,
                    'quality_score': 0.0,
            ...
            if not isinstance(reply_rules, dict):
                return {
                    'success': False,
                    'error': f'reply_rules must be a dictionary, got {type(reply_rules).__name__}',
                    'reply': None,
                    'quality_score': 0.0,
                    'processing_time_ms': 0,
                    'token_usage': 0,
            ...
            # 답글 생성 가능 여부 확인
            if not self._should_generate_reply(review, reply_rules):
                return {
                    'success': False,
            ...
            system_prompt = self._create_system_prompt(reply_rules)
            ...
            is_valid, quality_score = self._validate_reply(
                generated_reply, review, reply_rules
            )
            ...
            # 매장 정책에 맞게 답글 조정
            final_reply = self._apply_store_formatting(generated_reply, reply_rules)
    ...
    def _should_generate_reply(
        ...
        # 자동 답글 기능 활성화 확인
        if not reply_rules.get('auto_reply_enabled', True):
            return False
        
        return True
    
    def _create_system_prompt(self, reply_rules: Dict[str, Any]) -> str:
        """시스템 프롬프트 생성 - AI의 자율성 강화"""
        store_name = reply_rules.get('store_name', '우리 매장') or '우리 매장'
        role = reply_rules.get('role', '친절한 사장님') or '친절한 사장님'
        tone = reply_rules.get('tone', '친근하고 따뜻한') or '친근하고 따뜻한'
        greeting_start = reply_rules.get('greeting_start', '안녕하세요') or '안녕하세요'
        greeting_end = reply_rules.get('greeting_end', '감사합니다') or '감사합니다'
        max_length = reply_rules.get('max_length', 300) or 300
...
- 부적절한 표현 사용 금지"""
    ...
    def _create_prompt(
        ...
        """프롬프트 생성 (리뷰 내용 반영)"""
        rating = review.get('rating')
        content = review.get('review_content', '') or ''
        menu = review.get('ordered_menu', '') or ''
        name = review.get('review_name', '고객') or '고객'
        delivery_review = review.get('delivery_review', '') or ''
...
고객명: {name}님"""
    ...
    def _apply_store_formatting(
        ...
        # None 체크 및 기본값 처리
        greeting_start = reply_rules.get('greeting_start') or ''
        greeting_end = reply_rules.get('greeting_end') or ''
    ...
    def _validate_reply(
        ...
        # 품질 임계점 확인 (더 엄격하게 조정)
        threshold = reply_rules.get('manual_review_threshold', 0.5) or 0.5  # 0.3에서 0.5로 상향
        if score < threshold:
            is_valid = False
    ...
    async def _analyze_review_for_boss_attention(
        ...
        try:
            review_content = review.get('review_content', '') or ''
            rating = review.get('rating')
            ordered_menu = review.get('ordered_menu', '') or ''
            delivery_review = review.get('delivery_review', '') or ''
...
중요: 단순한 칭찬이나 일반적인 피드백은 false로 판단하세요."""
            ...
            try:
                ...
                return self._check_boss_review_needed_fallback(review)
    ...
    def _check_boss_review_needed_fallback(
        ...
        rating = review.get('rating')
        review_content = (review.get('review_content', '') or '').lower()
    ...
    async def regenerate_reply(
        ...
        try:
            result = await self.generate_reply(review, reply_rules)
...
def test_ai_reply():
    ...
    import asyncio
    reply = asyncio.run(generator.generate_reply(test_review, test_rules))
    
    if reply.get('success'):
        print("=== AI 생성 답글 ===")
        print(reply['reply'])
        print(f"\n글자 수: {len(reply['reply'])}자")
        print(f"품질 점수: {reply['quality_score']:.2f}")
    else:
        print(f"답글 생성 실패: {reply.get('error')}")
