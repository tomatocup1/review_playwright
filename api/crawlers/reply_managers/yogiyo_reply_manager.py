...
class YogiyoReplyManager:
    ...
    async def write_and_submit_reply(self, reply_content: str) -> bool:
        ...
        try:
            ...
            # 텍스트박스 찾기
            textarea = await self.page.wait_for_selector('textarea.ReviewReply__CustomTextarea-sc-1536a88-4', timeout=5000)
            ...
            # 등록 버튼 찾기
            register_button = await self.page.query_selector('button.sc-bczRLJ.ifUnxI.sc-eCYdqJ.hsiXYt:has-text("등록")')
            
            if not register_button:
                # 백업 선택자로 다시 시도
                register_button = await self.page.query_selector('button:has(span:has-text("등록"))')
